...
assert.match(ui, /function showYummySelect\(params, snapshot\)[\s\S]{0,1200}restoreAfterClose\(\)/);
assert.match(ui, /function showYummySelect\(params, snapshot\)[\s\S]{0,1600}params\.onSelect = function \(item\)[\s\S]{0,500}restoreAfterClose\(\)/);
assert.match(menu, /params\.yaniRestore = false/);
