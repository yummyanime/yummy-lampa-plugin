...
3. **Встроенный веб-плеер Alloha** — необязательный запасной вариант, по умолчанию выключен. Оригинальный веб-плеер открывается внутри Lampa, поэтому таймлайн Lampa, отслеживание прогресса и передача во внешний плеер недоступны.

Если ничего из этого не настроено, расширение показывает понятное предупреждение и не передаёт iframe несовместимому плееру. Другие доступные источники продолжают работать и располагаются выше в списке озвучек. Адреса resolver и Lampac, видимость источников, встроенный режим Alloha и предпочитаемый плеер настраиваются в разделе `Настройки → YummyAnime`.

Необязательная интеграция с YummyTV только открывает тайтл в отдельно установленном приложении YummyTV. Она не является resolver для Alloha и не нужна для остальных функций расширения.
