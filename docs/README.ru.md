...
Отключённые источники не показываются. VK по умолчанию включён: поддерживаемые страницы-обёртки VK преобразуются в прямой MP4/HLS-поток. Alloha и CVH по умолчанию выключены: для стабильного воспроизведения могут потребоваться дополнительные внешние компоненты. При включении любого из них расширение показывает предупреждение. Последнее успешно загруженное расписание доступно и при временном сбое API.
